...
def inversion_rudge(m, n, T, P, xtol = 1e-10):
    """
    Double spike inversion using method from Rudge et al. 20XX[1]
...
def _inversion_rudge_function(self, x, P, n, T, m):
    x  = x.reshape(3,-1)
    lambda_ = x[0]
    alpha = x[1]/(1-lambda_)
    beta = x[2]
...
def _inversion_rudge_jacobian(self, x, P, n, T, m):
...
def inversion_siebert(self, MS, ST, SP, Mass, Fnat_guess = 2, Fins_guess = -0.00001, loop_one_iterations = 3, loop_two_iterations=6):
    """
    Double spike inversion using method from Siebert et al. 20XX[1]
...
def inversion(measured, standard, spike, mass, inversion_ratios = None, method = 'rudge', **method_kwargs):
    """
    ...
    if method == 'Rudge': return inversion_rudge(m, n, T, np.log(P), **method_kwargs)
    elif method == 'Siebert': return inversion_siebert(m, n, T, P)
...
def johnson_nyquist_noise(voltages, resistors = None, integration_time = None, include_counting_statistics = True,
    ...
    if include_counting_statistics:
        c_noise = np.sqrt(1 / (voltages * cpv * integration_time))
        return np.sqrt(c_noise**2 + t_noise **2)*voltages
    else:
        return t_noise

def uncertainty_grid(standard, spike1, spike2, mass, *, isotopes = None, max_voltage = 10, resistors = None,
...
def plot_uncertainty_grid(*args, **kwargs):
    x, y, z, best = uncertianty_grid(*args, **kwargs)
...
def cocktail_list(standard, mass, spikes = None, isotopes = None, output_mass_ratio = None, output_multiple = 1000, **kwargs):
    if isinstance(standard, _dtypes.IsotopeArray):
        if isotopes is None: isotopes = standard.keys()
    elif isinstance(standard, _dtypes.IsopyDict):
